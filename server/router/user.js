...
exports.login = async (req, res) => {
    ...
    let sqlSelect =`select user_id,username,password from blog_user where user_id=?`;
    mysql.query(sqlSelect,[user_id]).then(result=>{
        if(result.length>0){
            let user=result[0];
            ...
            if(isMatch){
                ...
                return res.json({result:{user_id:0,username:""},msg:"密码错误"})
            }
        }else{
            return res.json({result:{user_id:0,username:""},msg:"用户不存在"})
        }
    })
